...
public:
	...
	static void QueryDefaultCatalog(DuckDB *db, bool *read_correct, int thread_id) {
		duckdb::vector<duckdb::string> random_default_views {
		    "pragma_database_list", "sqlite_master",      "sqlite_schema",  "sqlite_temp_master",
		    "sqlite_temp_schema",   "duckdb_constraints", "duckdb_columns", "duckdb_indexes",
		    "duckdb_schemas",       "duckdb_tables",      "duckdb_types",   "duckdb_views"};
	...
	static void QueryDefaultCatalogFunctions(DuckDB *db, bool *read_correct, int thread_id) {
		duckdb::vector<duckdb::string> random_queries {
