...
class PrefilterTree {
 public:
  ...
  void Compile(std::vector<duckdb::string>* atom_vec);
 ...
 private:
  ...
  typedef std::map<duckdb::string, Prefilter*> NodeMap;
  ...
  // PrefilterTree.
  void AssignUniqueIds(NodeMap* nodes, std::vector<duckdb::string>* atom_vec);
  ...
  duckdb::string NodeString(Prefilter* node) const;

  // Recursively constructs a readable prefilter string.
  duckdb::string DebugNodeString(Prefilter* node) const;
