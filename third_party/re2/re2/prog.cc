...
static duckdb::string ProgToString(Prog* prog, Workq* q) {
  duckdb::string s;
...
static duckdb::string FlattenedProgToString(Prog* prog, int start) {
  duckdb::string s;
...
duckdb::string Prog::DumpUnanchored() {
  if (did_flatten_)
...
duckdb::string Prog::DumpByteMap() {
  duckdb::string map;
  for (int c = 0; c < 256; c++) {
    int b = bytemap_[c];
    int lo = c;
