...
//      duckdb::string contents = ...;     // Fill string somehow
...
class RE2 {
 public:
  ...
  // Need to have the const char* and const duckdb::string& forms for implicit
  // conversions when passing string literals to FullMatch and PartialMatch.
  // Otherwise the StringPiece form would be sufficient.
#ifndef SWIG
  RE2(const char* pattern);
  RE2(const duckdb::string& pattern);
#endif
  ...
  //   RE2 re("ab*c?d+");
  //   re.pattern();    // "ab*c?d+"
  const duckdb::string& pattern() const { return pattern_; }

  // If RE2 could not be created properly, returns an error string.
  // Else returns the empty string.
  const duckdb::string& error() const { return *error_; }
  ...
  const duckdb::string& error_arg() const { return error_arg_; }
 ...
 public:
  ...
  //
  // You can pass in a "const char*" or a "duckdb::string" for "text".
  // You can pass in a "const char*" or a "duckdb::string" or a "RE2" for "re".
  //
  // The provided pointer arguments can be pointers to any scalar numeric
  // type, or one of:
  //    duckdb::string     (matched piece is copied to string)
  ...
  // text.  E.g.,
  //
  //   duckdb::string s = "yabba dabba doo";
  ...
  // false otherwise.
  static bool Replace(duckdb::string* str,
                      const RE2& re,
  ...
  static int GlobalReplace(duckdb::string* str,
                           const RE2& re,
                           const StringPiece& rewrite);
  ...
  // may become:
  //           1\.5\-2\.0\?
  static duckdb::string QuoteMeta(const StringPiece& unquoted);
  ...
  bool PossibleMatchRange(duckdb::string* min, duckdb::string* max,
                          int maxlen) const;
  ...
  const std::map<duckdb::string, int>& NamedCapturingGroups() const;

  // Return a map from capturing indices to names.
  // The map has no entries for unnamed groups.
  // Only valid until the re is deleted.
  const std::map<int, duckdb::string>& CapturingGroupNames() const;
  ...
  // be sucessful.
  bool Rewrite(duckdb::string* out,
               const StringPiece& rewrite,
 ...
 private:
  ...
  duckdb::string   pattern_;          // string regular expression
  Options       options_;          // option flags
  duckdb::string   prefix_;           // required prefix (before regexp_)
  ...
  mutable const duckdb::string*  error_;    // Error indicator
                                         // (or points to empty string)
  mutable ErrorCode      error_code_;    // Error code
  mutable duckdb::string    error_arg_;     // Fragment of regexp showing error

  // Map from capture names to indices
  mutable const std::map<duckdb::string, int>* named_groups_;

  // Map from capture indices to names
  mutable const std::map<int, duckdb::string>* group_names_;
...
#define MAKE_PARSER(type, name)            \
  ...
  MAKE_PARSER(duckdb::string,        parse_string)
