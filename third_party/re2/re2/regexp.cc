...
duckdb::string RegexpStatus::CodeText(enum RegexpStatusCode code) {
  if (code < 0 || code >= arraysize(kErrorStrings))
    code = kRegexpInternalError;
  return kErrorStrings[code];
}

duckdb::string RegexpStatus::Text() const {
  if (error_arg_.empty())
...
class NamedCapturesWalker : public Regexp::Walker<Ignored> {
 public:
  ...
  std::map<duckdb::string, int>* TakeMap() {
    std::map<duckdb::string, int>* m = map_;
    map_ = NULL;
    return m;
  }
  ...
  Ignored PreVisit(Regexp* re, Ignored ignored, bool* stop) {
    if (re->op() == kRegexpCapture && re->name() != NULL) {
      ...
      if (map_ == NULL)
        map_ = new std::map<duckdb::string, int>;
...
class CaptureNamesWalker : public Regexp::Walker<Ignored> {
 public:
  ...
  std::map<int, duckdb::string>* TakeMap() {
    std::map<int, duckdb::string>* m = map_;
    map_ = NULL;
    return m;
  }
  ...
  Ignored PreVisit(Regexp* re, Ignored ignored, bool* stop) {
    if (re->op() == kRegexpCapture && re->name() != NULL) {
      // Allocate map once we find a name.
      if (map_ == NULL)
        map_ = new std::map<int, duckdb::string>;
...
bool Regexp::RequiredPrefix(duckdb::string* prefix, bool* foldcase,
                            Regexp** suffix) {
